// =============================================================================
// Zernio SaaS Agent Worker (v10: Hybrid Execution, Quota Guard, Live Stats & RAG)
// =============================================================================
...
// حد الرسائل المجاني الافتراضي
const DEFAULT_FREE_QUOTA = 100;

// إعدادات CORS للسماح لتطبيقك بالتواصل مع السيرفر
const corsHeaders = {
  'Access-Control-Allow-Origin': '*',
  'Access-Control-Allow-Methods': 'GET, POST, OPTIONS',
  'Access-Control-Allow-Headers': 'Content-Type, Authorization, x-zernio-key, x-admin-key',
};

// -----------------------------------------------------------------------------
// 1) API PROXY & MANAGEMENT ENDPOINTS (مسارات الواجهة والإدارة)
// -----------------------------------------------------------------------------
async function handleApiRequests(request, env, url) {
    ...
    // 1. مسار حفظ وقراءة تعليمات العميل (System Prompt)
    if (path === '/api/set-prompt' && request.method === 'POST') {
        const body = await request.json().catch(() => ({}));
        if (!body.prompt) return jsonResponse({ error: 'حقل prompt مطلوب' }, 400);
        await env.ZERNIO_KV.put('custom_agent_prompt', body.prompt);
        return jsonResponse({ ok: true, message: 'تم حفظ التعليمات في ذاكرة البوت بنجاح' });
    }

    if (path === '/api/get-prompt' && request.method === 'GET') {
        const prompt = await env.ZERNIO_KV.get('custom_agent_prompt') || '';
        return jsonResponse({ ok: true, prompt });
    }

    // 2. مسار حفظ وقراءة مستندات الـ RAG وقواعد المعرفة
    if (path === '/api/set-rag' && request.method === 'POST') {
        const body = await request.json().catch(() => ({}));
        if (!body.ragData && body.ragData !== "") return jsonResponse({ error: 'حقل ragData مطلوب' }, 400);
        await env.ZERNIO_KV.put('rag_knowledge_base', typeof body.ragData === 'string' ? body.ragData : JSON.stringify(body.ragData));
        return jsonResponse({ ok: true, message: 'تم تحديث قاعدة المعرفة (RAG) بنجاح' });
    }

    if (path === '/api/get-rag' && request.method === 'GET') {
        const ragData = await env.ZERNIO_KV.get('rag_knowledge_base') || '';
        return jsonResponse({ ok: true, ragData });
    }

    // 3. مسار جلب الإحصائيات الحية (Live Analytics Endpoint)
    if (path === '/api/stats' && request.method === 'GET') {
        const stats = await getLiveStats(env);
        return jsonResponse({ ok: true, stats });
    }

    // 4. مسار تصفير وإعادة تعيين الخادم (Worker Reset) لنقل الملكية
    if (path === '/api/reset-worker' && request.method === 'POST') {
        await resetWorkerState(env);
        return jsonResponse({ ok: true, message: 'تمت إعادة ضبط وتصفير السيرفر بالكامل وهو جاهز لمستخدم جديد' });
    }

    // 5. مسارات الـ OAuth لفيسبوك وإنستغرام
    if (path === '/api/auth/facebook' && request.method === 'GET') {
    ...
    if (path === '/api/auth/facebook/select' && request.method === 'POST') {
        const body = await request.json().catch(() => ({}));
    ...
    if (path === '/api/auth/instagram' && request.method === 'GET') {
        const redirectUrl = url.searchParams.get('redirect_url') || '';
    ...
    if (path === '/api/auth/instagram/accounts' && request.method === 'GET') {
    ...
    if (path === '/api/auth/instagram/select' && request.method === 'POST') {
        const body = await request.json().catch(() => ({}));
        body.profileId = PROFILE_ID;
...
// -----------------------------------------------------------------------------
// 2) نظام الإحصائيات وإدارة الكوتا وتصفير الخادم
// -----------------------------------------------------------------------------
async function getLiveStats(env) {
    const messagesReceived = parseInt(await env.ZERNIO_KV.get('stat_msg_received') || '0', 10);
    const messagesReplied = parseInt(await env.ZERNIO_KV.get('stat_msg_replied') || '0', 10);
    const commentsHandled = parseInt(await env.ZERNIO_KV.get('stat_comments_handled') || '0', 10);
    const quotaUsed = parseInt(await env.ZERNIO_KV.get('messages_usage_count') || '0', 10);
    const quotaLimit = parseInt(await env.ZERNIO_KV.get('quota_limit') || String(DEFAULT_FREE_QUOTA), 10);
    const plan = await env.ZERNIO_KV.get('user_plan') || 'free';
    const lastActiveAt = await env.ZERNIO_KV.get('stat_last_active') || isoNow();
    const avgResponseTimeMs = parseInt(await env.ZERNIO_KV.get('stat_avg_response_ms') || '1650', 10);
...
async function incrementStats(env, type, durationMs) {
    try {
        if (type === 'message') {
            const cur = parseInt(await env.ZERNIO_KV.get('stat_msg_replied') || '0', 10) + 1;
            await env.ZERNIO_KV.put('stat_msg_replied', String(cur));
            
            const usage = parseInt(await env.ZERNIO_KV.get('messages_usage_count') || '0', 10) + 1;
            await env.ZERNIO_KV.put('messages_usage_count', String(usage));
        } else if (type === 'comment') {
            const cur = parseInt(await env.ZERNIO_KV.get('stat_comments_handled') || '0', 10) + 1;
            await env.ZERNIO_KV.put('stat_comments_handled', String(cur));
        }

        if (durationMs) {
            await env.ZERNIO_KV.put('stat_avg_response_ms', String(Math.round(durationMs)));
        }
...
async function resetWorkerState(env) {
    // تصفير الذاكرة ليعود السيرفر جديداً تماماً
    await env.ZERNIO_KV.delete('custom_agent_prompt');
    await env.ZERNIO_KV.delete('rag_knowledge_base');
    await env.ZERNIO_KV.delete('messages_usage_count');
    await env.ZERNIO_KV.delete('stat_msg_received');
    await env.ZERNIO_KV.delete('stat_msg_replied');
    await env.ZERNIO_KV.delete('stat_comments_handled');
    await env.ZERNIO_KV.delete('stat_last_active');
    await env.ZERNIO_KV.put('quota_limit', String(DEFAULT_FREE_QUOTA));
    await env.ZERNIO_KV.put('user_plan', 'free');
}
...
// -----------------------------------------------------------------------------
// 4) كتالوج العمليات المسموحة (CALL HANDLERS)
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// 5) دمج الـ Prompt والـ RAG مع تعليمات النظام
// -----------------------------------------------------------------------------
async function getDynamicSystemInstruction(env) {
  ...
  let ragData = await env.ZERNIO_KV.get('rag_knowledge_base');
  let ragSection = "";
  if (ragData && ragData.trim() !== "") {
    ragSection = [
      "",
      "=== قاعدة المعرفة والأسعار والسياسات الرسمية (RAG Data) ===",
      "استخرج إجاباتك ومعلومات الأسعار والمنتجات من البيانات التالية بدقة:",
      ragData,
      ""
    ].join("\n");
  }
  ...
  return [
    ...
    "أنت وكيل ذكي بيرد على رسائل הـ Direct Messages والتعليقات.",
    ...
    '2) {"action": "final", "text": "..."}',
    "",
    "── كتالوج הـ DM (event = message.received) ──",
...
async function callWorkersAiTurn(env, contents, systemInstruction, combo) {
  ...
  if (!res.ok) throw new Error("Workers AI Error");
...
// -----------------------------------------------------------------------------
// 6) حلقة تنفيذ الوكيل (Agent Loop)
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// 7) معالجة الأحداث وحماية الكوتا (Quota & Message Processing)
// -----------------------------------------------------------------------------
...
async function handleZernioEvent(env, rawBody, payload, receivedAt) {
  ...
  if (eventType === "message.received" || eventType === "comment.received") {
    // 1. تسجيل استلام رسالة جديدة في الإحصائيات
    const curReceived = parseInt(await env.ZERNIO_KV.get('stat_msg_received') || '0', 10) + 1;
    await env.ZERNIO_KV.put('stat_msg_received', String(curReceived));

    // 2. فحص كوتا الـ 100 رسالة للحسابات المجانية (Quota Guard)
    const plan = await env.ZERNIO_KV.get('user_plan') || 'free';
    const quotaUsed = parseInt(await env.ZERNIO_KV.get('messages_usage_count') || '0', 10);
    const quotaLimit = parseInt(await env.ZERNIO_KV.get('quota_limit') || String(DEFAULT_FREE_QUOTA), 10);

    if (plan === 'free' && quotaUsed >= quotaLimit) {
      await logActivity(env, {
        eventId,
        event: eventType,
        outcome: "quota-exceeded",
        note: `تم استهلاك الحد المسموح (${quotaUsed}/${quotaLimit}) - تم إيقاف المعالجة.`
      });
      return; // توقف عن استهلاك الذكاء الاصطناعي
    }

    let rawEventText = rawBody;
    
    // 3. جلب السياق التلقائي
    ...
    // 4. استدعاء الوكيل الذكي للرد
    const trace = await runAgentLoop(env, rawEventText, eventId);
    
    // 5. احتساب الرد في الإحصائيات وتحديث متوسط وقت الرد
    const durationMs = Date.now() - startTime;
    if (trace && trace.ok) {
        await incrementStats(env, eventType === "message.received" ? 'message' : 'comment', durationMs);
    }
  }
}

// -----------------------------------------------------------------------------
// 8) نقطة الدخول الهجينة (Hybrid Direct + Queue Fallback)
// -----------------------------------------------------------------------------
export default {
  async fetch(request, env, ctx) {
    ...
    // 2. توجيه مسارات الـ API للواجهة الأمامية
    if (url.pathname.startsWith('/api/')) {
        return await handleApiRequests(request, env, url);
    }

    // 3. استقبال ومعالجة الـ Webhook من Zernio
    if (request.method === "POST" && url.pathname === "/webhook/zernio") {
      ...
      // معالجة فورية وسريعة (Direct Processing) لرد فوري أقل من ثانية
      try {
        await handleZernioEvent(env, rawBody, payload, receivedAt);
        return jsonResponse({ ok: true, direct: true });
      } catch (err) {
        console.error("Direct processing failed, falling back to Queue:", err);
        
        // في حالة الفشل فقط: يتم تحويل الرسالة لطابور الطوارئ (Queue) لإعادة المحاولة
        if (env.EVENTS_QUEUE) {
          await env.EVENTS_QUEUE.send({ rawBody, payload, receivedAt });
        }
        return jsonResponse({ ok: true, queuedForRetry: true });
      }
    }

    return textResponse("Bedaya Agent Worker (v10: Hybrid) Running.");
  },

  // معالج طابور الطوارئ (Cloudflare Queue Consumer)
