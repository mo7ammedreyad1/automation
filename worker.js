...
/uth/// -----------------------------------------------------------------------------
// 1) API PROXY: Endpoints الخاصة بالتطبيق (مصححة ومؤمنة 100%)
// -----------------------------------------------------------------------------
async function handleApiRequests(request, env, url) {
    const path = url.pathname;
    
    // قراءة المفاتيح المكتوبة في أول الكود أو المسجلة في Cloudflare
    const API_KEY = (typeof WORKER_ZERNIO_API_KEY !== 'undefined' ? WORKER_ZERNIO_API_KEY : env.ZERNIO_API_KEY || '').trim();
    const PROFILE_ID = (typeof WORKER_ZERNIO_PROFILE_ID !== 'undefined' ? WORKER_ZERNIO_PROFILE_ID : env.ZERNIO_PROFILE_ID || '').trim();
    ...
    // 3. مسار تأكيد ربط صفحة الفيسبوك (Facebook Page Select)
    ...
    // 4. مسار جلب رابط تفويض الإنستغرام (Instagram OAuth Link)
    ...
    // 5. مسار جلب حسابات الإنستغرام (حل مشكلة الـ 404 بتشفير الـ tempToken)
    if (request.method === 'GET' && path === '/api/auth/instagram/accounts') {
        const tempToken = url.searchParams.get('tempToken') || '';
        
        // 🔥 تشفير التوكن لمنع انكسار الرابط وخطأ 404
    ...
    // 6. مسار تأكيد ربط حساب الإنستغرام (Instagram Account Select)
    ...
    return new Response(JSON.stringify({ error: 'المسار غير موجود (Endpoint not found)' }), { 
        status: 404, 
        headers: corsHeaders 
    });
          }
